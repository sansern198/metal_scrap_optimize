...
# ---------------------- Image/Camera switch ----------------------
USE_IMAGE = True
IMAGE_SOURCE = os.path.join(script_dir, "img/img1.jpg") 

# Measurement parameters
blue_Lower = (90, 120, 120)
blue_Upper = (150, 255, 255)
ROTATE_ANGLE_DEG = 2.4
CROP_SLICE = (slice(480, 2590), slice(50, 4000))
PIXEL_MM_RATIO_W = 0.5319
PIXEL_MM_RATIO_H = 0.5284
...
def cal_dim(contours, hierarchy, px2mm_x, px2mm_y):
    ...
    dists.sort()
    h_px = (dists[0] + dists[1]) / 2.0
    w_px = (dists[2] + dists[3]) / 2.0

    width_mm  = w_px * px2mm_x
    height_mm = h_px * px2mm_y
    area_mm2  = width_mm * height_mm

    # angle = cv2.minAreaRect(max_cnt)[2]
...
class LivePanel(QtWidgets.QFrame):
    def __init__(self, left_w, mid_h, s):
        ...
        # NOTE: Removed status row (Power/Camera/Processing/Network) as requested
...
class RightPanel(QtWidgets.QFrame):
    def __init__(self, right_w, s):
        ...
        # ↓↓↓ Reduced font sizes for measurement KPIs as requested ↓↓↓
...
class MainWindow(QtWidgets.QMainWindow):
        ...
        if USE_IMAGE:
            if not os.path.exists(IMAGE_SOURCE):
                ...
                if self.image_bgr is None:
                    ...
                    # เริ่มอัปเดต live feed จากรูปภาพ (ใส่ timestamp ให้เหมือนกล้อง)
            ...
            # โหมดวิดีโอ: ใช้กล้องผ่าน GStreamer ตามเดิม
    ...
    def on_camera_online(self, ok: bool):
        # Removed live panel status LEDs; keep optional header indicator
    ...
    def handle_measure(self):
        ...
        # Removed self.live.proc_dot usage
        try:
            if USE_IMAGE:
                if self.image_bgr is None:
    ...
    # ---------------------- NEW: ฟังก์ชันประมวลผลฝั่งรูปภาพ ----------------------
    def run_measurement_image(self, image_bgr: np.ndarray):
        """
        ประมวลผลรูปภาพคงที่ (image mode) โดยคง pipeline เดิมทั้งหมด
        แยกฟังก์ชันเพื่อชัดเจนและเผื่ออนาคต (เช่น รองรับโฟลเดอร์หลายไฟล์)
        """
    ...
    def run_measurement(self, frame_bgr):
        # ตัดบรรทัดนี้ทิ้ง: frame_HD = resize_to_target(frame_bgr)
        frame_HD = frame_bgr
        ...
        # glare removal + blue background masking
        ...
        # ใช้ external contours แบบเดียวกับตัวอย่าง
        ...
        cv2.putText(output, f"Area:  {object_area:.1f} mm2", (cX - 120, cY - 90), cv2.FONT_HERSHEY_PLAIN, 4, (0,0,0), 4)
        cv2.putText(output, f"Width: {object_width:.1f} mm", (cX - 120, cY - 20), cv2.FONT_HERSHEY_PLAIN, 4, (0,0,0), 4)
        cv2.putText(output, f"Height:{object_height:.1f} mm", (cX - 120, cY + 40),  cv2.FONT_HERSHEY_PLAIN, 4, (0,0,0), 4)
    ...
    def closeEvent(self, event: QtGui.QCloseEvent) -> None:
        try:
            ...
            # หยุด image timer กรณีอยู่โหมดรูปภาพ
